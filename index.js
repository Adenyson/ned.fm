...
client.on('message', async message => { // Evento de mensagem do Discord
    ...
    if (command === 'register') { // Se o comando digitado for "register"
        ...
        lastfm.nowPlaying({sk: session}, (err, nowPlaying) => { // Busca a música atual tocando no LastFM
            if (err) {
                ...
                message.reply(`Música atual: ${nowPlaying.artist['#text']} - ${nowPlaying.name}`); // Envia a música atual para o usuário 

              } 
        }); 
     } else if (command === 'play') {
  // Se o comando digitado for "play"
  ...
  lastfm.track.scrobble({sk: session, track, timestamp: Math.floor(Date.now() / 1000)}, (err) => {
